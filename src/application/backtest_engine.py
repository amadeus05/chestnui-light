...
from src.data.etl.contexts import attach_funding_context, attach_open_interest_context, attach_premium_index_context
...
def _enrich_main_ohlcv(repository: HistoricalKlineRepository, symbol: str, main_df: pd.DataFrame) -> pd.DataFrame:
    ...
    premium_index_df = repository.load_premium_index_klines(symbol, str(TIMEFRAME))
    open_interest_df = repository.load_open_interest(symbol, str(TIMEFRAME))
    df = main_df.copy()
...
def build_feature_row_at_time(
    ...
    """Собирает строку фич на `analysis_ts` через MasterFeatureBuilder (как ETL), с кросс-секцией по всем символам."""
    ...
    try:
        ...
        result = builder.build(base_map, htf_map)
...
def backtest(
    ...
    reporter = BacktestReporter(
        ...
        default_equity_chart_path=EQUITY_CURVE_PATH,
