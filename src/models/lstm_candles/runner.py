...
class LstmCandlesRunner(BaseModelRunner):
    ...
    def run_walk_forward(self, argv: Sequence[str] | None = None) -> None:
        ...
        if args.dry_run:
            plan = LstmCandlesWalkForwardRunner().plan(request)
            self.print_walk_forward_plan(plan)
            return

        result = LstmCandlesWalkForwardRunner().run(request)
    ...
    def print_walk_forward_plan(plan) -> None:
        print("=" * 72)
        print("CANDLE LSTM WALK-FORWARD PLAN (DRY RUN)")
        ...
        print(f"Candle rows: {plan.candle_rows}")
        print(f"Candle features: {plan.feature_count}")
