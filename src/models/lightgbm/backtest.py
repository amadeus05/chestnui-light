...
class LightGbmBacktestRunner:
    """Replay backtest for LightGBM OOS predictions stored in the new artifact layout."""

    def __init__(self, spec: ModelSpec, artifact_store: ArtifactStore | None = None):
        self.spec = spec
        self.artifact_store = artifact_store or ArtifactStore()

    def run(self, request: LightGbmBacktestRequest) -> None:
    ...
    def to_backtest_features_meta(metadata: dict, predictions: pd.DataFrame) -> dict:
        ...
        return {
            ...
            "task_type": f"{metadata.get('model_key', 'lightgbm')}_walk_forward_oos_replay",
