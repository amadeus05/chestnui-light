...
class LstmFeaturePredictor(ModelPredictor):
    spec: ModelSpec
    model: Any
    ...
    def predict(self, model_input: ModelInput) -> Prediction:
        if not isinstance(model_input, LstmFeatureSequenceInput):
            raise TypeError("LstmFeaturePredictor expects LstmFeatureSequenceInput.")
        if self.standardizer is None:
            raise RuntimeError("LSTM feature predictor requires fitted standardizer.")
        ...
        if len(proba) == 0:
            raise ValueError("LSTM prediction input produced no sequences.")
...
def build_lstm_feature_model(input_size: int, config: LstmTrainingConfig) -> LSTMClassifier:
