...
from src_refactor.infrastructure.models.lstm_features.predictor import LstmFeaturePredictor, build_lstm_feature_model


@dataclass(frozen=True, slots=True)
class LstmFeatureTrainer(ModelTrainer):
    def train(
        self,
        ...
        if not isinstance(train_input, LstmFeatureSequenceInput):
            raise TypeError("LstmFeatureTrainer expects LstmFeatureSequenceInput.")
        if train_input.targets is None:
            raise ValueError("LSTM feature training requires targets.")
        if len(train_input.sequence) < LstmTrainingConfig.from_metadata(config.model.metadata).min_train_rows:
            raise ValueError("Not enough LSTM feature sequences to train.")

        training_config = LstmTrainingConfig.from_metadata(config.model.metadata)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model, standardizer, training_meta = train_lstm_classifier(
            train_input.sequence,
            train_input.targets,
            training_config,
            device=device,
        )
        ...
        return ModelArtifact(
            ...
            metadata={
                ...
                "model_args": {
                    "input_size": int(train_input.sequence.shape[-1]),
                ...
                "rows": int(len(train_input.sequence)),
            },
        )


@dataclass(frozen=True, slots=True)
class LstmFeatureArtifactStore(ModelArtifactStore):
    ...
    def load_predictor(self, spec: ModelSpec, fold_id: int | None = None) -> ModelPredictor:
        ...
        training_config = LstmTrainingConfig.from_metadata(payload.get("training", {}))
        model_args = payload.get("model_args", {})
        model = build_lstm_feature_model(int(model_args["input_size"]), training_config)
